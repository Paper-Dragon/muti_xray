...
1. 选项 `4` 初始化配置，选择 socks5 协议
2. 端口范围输入 `30000-40000`
3. 在 socks5 高级配置中选择"否"（不使用顺序端口）
4. 选项 `8` 列出节点，确认端口在 30000-40000 范围内

**顺序端口 + 指定起始端口：**

1. 选项 `4` 初始化配置，同时选择 socks5 和 shadowsocks
2. 端口范围保持默认（顺序模式时不使用该范围）
3. socks5 高级配置中选择"顺序分配端口"，起始端口输入 `20001`
4. shadowsocks 中选择"顺序分配端口"，起始端口输入 `30001`
5. 选项 `8` 列出节点，确认：
   - socks5 端口为 20001, 20002, 20003...（每张网卡递增）
   - shadowsocks 端口为 30001, 30002, 30003...（各协议独立计数）

### 测试追加协议功能

1. 先通过选项 `4` 初始化配置，选择 socks5 协议（顺序端口 20001）
2. 选项 `8` 列出节点，确认 socks5 端口从 20001 开始
3. 选项 `5` 追加协议，选择 vmess，端口范围设为 `40000-50000`（随机模式）
4. 选项 `8` 列出节点，确认 vmess 端口在 40000-50000 范围内
5. 再次选项 `5` 追加 trojan，选择"顺序分配端口"，起始端口 `60001`
6. 选项 `8` 确认 trojan 端口为 60001, 60002, 60003...
